...
const m = {
    spawn() {
        ...
        jumpSensor = Bodies.rectangle(0, 46, 50, 6, {
   ...
   death() {
    ...
    setFieldRegen() {
        ...
        if (m.fieldMode === 0 || m.fieldMode === 6) m.fieldRegen += 0.002 * m.coupling
    ...
    pushMass(who, fieldBlockCost = (0.025 + Math.sqrt(who.mass) * Vector.magnitude(Vector.sub(who.velocity, player.velocity)) * 0.002) * m.fieldShieldingScale) {
        if (m.energy > m.minEnergyToDeflect) { //shield needs at least some of the cost to block
            if (who.isShielded) fieldBlockCost *= 1; //shielded mobs take more energy to block
        ...
        effect: () => {
            ...
            m.perfectPush = (isFree = false) => {
                if (m.fieldCDcycle < m.cycle) {
                    for (let i = 0, len = mob.length * 10; i < len; ++i) {
                        if (
    ...
    },
    {
        ...
        description: "use <strong class='color-f'>energy</strong> to nullify &nbsp;<strong style='letter-spacing: 7px;'>gravity</strong><br><strong>0.4x</strong> <strong class='color-defense'>damage taken</strong><br> 2.5x faster than normal <br><strong>6</strong> <strong class='color-f'>energy</strong> per second",
        fieldDrawRadius: 0,
        effect: () => {
            m.fieldFire = true;
                ...
                if (m.isHolding) {
                    ...
                    if (m.energy > DRAIN && m.fieldCDcycle < m.cycle) {
                        if (tech.isFlyFaster) {
                            ...
                            m.airSpeedLimit = 2500 // 7* player.mass * player.mass
                            m.FxAir = 0.01
                            ...
                            //control horizontal acceleration
                            m.airSpeedLimit = 2000 // 7* player.mass * player.mass
                            m.FxAir = 0.005
                            //control vertical acceleration
                            if (input.down) { //down
                                player.force.y -= 0.5 * player.mass * simulation.g;
    ...
    },
    {
        ...
        effect: () => {
            ...
            m.hold = function () {
                ...
                if (m.energy < 0.05 && !input.fire) m.fireCDcycle = m.cycle
                if (!input.fire) { //automatically cloak if not firing
                    // const drain = 0.02
            ...
            m.hold = function () {
                if (m.isHolding) {
                    ...
                    if (m.fieldCDcycle < m.cycle) {
                        ...
                        b.grapple({ x: m.pos.x + 80 * Math.cos(m.angle), y: m.pos.y + 80 * Math.sin(m.angle) }, m.angle)
    ...
    //************************************************************************************
    isShipMode: false,
    shipMode(thrust = 0.03, drag = 0.99, torque = 1.15, rotationDrag = 0.99) { //  m.shipMode() //thrust = 0.03, drag = 0.99, torque = 1.15, rotationDrag = 0.92
        if (!m.isShipMode) {
