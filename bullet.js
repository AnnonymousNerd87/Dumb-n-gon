...
const b = {
    ...
    harpoon(where, target, angle = m.angle, harpoonSize = 6, isReturn = false, totalCycles = 35, isReturnAmmo = true, thrust = 0.2) {
        const me = bullet.length;
        const returnRadius = 200 * Math.sqrt(harpoonSize)
        bullet[me] = Bodies.fromVertices(where.x, where.y, [{
            x: -80 * harpoonSize,
            y: 4 * harpoonSize,
            index: 0,
    ...
    missile(where, angle, speed, size = 1) {
        ...
        const thrust = 0.022 * bullet[me].mass * (tech.isMissileBig ? (tech.isMissileBiggest ? 0.3 : 0.7) : 1);
        Matter.Body.setVelocity(bullet[me], {
            x: m.Vx / 2 + speed * 2 * Math.cos(angle),
            y: m.Vy / 2 + speed * 2 * Math.sin(angle)
        });
        Composite.add(engine.world, bullet[me]); //add bullet to world
    },
    lastAngle: 0,
    ...
    plasma() {
        ...
        if (m.energy > DRAIN) {
            ...
            let range = tech.isPlasmaRange * 30 * (240 + (m.crouch ? 800 : 600) * Math.sqrt(Math.random())) //+ 100 * Math.sin(m.cycle * 0.3);
    ...
    laser(where = {
        ...
        x: where.x + 30000 * Math.cos(m.angle),
        y: where.y + 30000 * Math.sin(m.angle)
    }, dmg = tech.laserDamage, reflections = tech.laserReflections * 5, isThickBeam = false, push = 1) {
        const reflectivity = 1 - 1 / (reflections * 5)
        ...
        if (isThickBeam) {
            for (let i = 1, len = path.length * 3; i < len; ++i) {
    ...
    mine(where, velocity, angle = 0) {
        ...
        bullet[bIndex] = Bodies.rectangle(where.x, where.y, 90, 32, {
            angle: angle,
            friction: 1,
            frictionStatic: 1,
            frictionAir: 0,
            ...
            onEnd() {
                if (this.isArmed && !tech.isMineSentry) {
                    if (tech.isFoamMine) {
                        ...
                        for (let i = 0; i < 48; i++) {
                            const radius = 26 + 16 * Math.random()
                            const velocity = { x: 6 * Math.random(), y: 0 }
                                ...
                                for (let i = 0, len = mob.length; i < len; ++i) {
                                    if (
                                        ...
                                        if (tech.isMineSentry) {
                                            this.lookFrequency = Math.floor(7 + 2 * b.fireCDscale + 10 * (tech.oneSuperBall && tech.isSuperMine) + Math.floor(3 * Math.random()))
                                            ...
                                            this.do = function () { //overwrite the do method for this bullet
                                                ...
                                                if (!(simulation.cycle % this.lookFrequency)) { //find mob targets
                                                    if (tech.isFoamMine) {
                                                        this.shots -= 0.1 * b.targetedFoam(this.position, 1, 21 + 7 * Math.random(), 1200, false)
                                                        b.targetedFoam(this.position, 1, 21 + 7 * Math.random(), 1200, false)
                                                    } else if (tech.isSuperMine) {
                                                        const cost = tech.oneSuperBall ? 2 : 0.7
                                                        this.shots -= cost * 0.2 * b.targetedBall(this.position, 1, 42 + 12 * Math.random(), 1200, false)
                                                        for (let i = 0, len = tech.extraSuperBalls / 2; i < len; i++) {
                                                            if (Math.random() < 0.33) b.targetedBall(this.position, 1, 42 + 12 * Math.random(), 1200, false)
                                                        }
                                                    } else {
                                                        this.shots -= 0.2 * b.targetedNail(this.position, 1, 45 + 5 * Math.random(), 1100, 2.3)
                                                    }
    ...
    worm(where, isFreeze = tech.isSporeFreeze) { //used with the tech upgrade in mob.death()
        const bIndex = bullet.length;
        const wormSize = 12 + tech.wormSize * 33.6 * Math.random()
    ...
    flea(where, velocity, radius = 18 + 9 * Math.random() + 30 * tech.wormSize * Math.random()) {
        const me = bullet.length;
    ...
    drone(where = { x: m.pos.x + 30 * Math.cos(m.angle) + 20 * (Math.random() - 0.5), y: m.pos.y + 30 * Math.sin(m.angle) + 20 * (Math.random() - 0.5) }, speed = 1) {
        ...
        const RADIUS = (18 + 12 * Math.random())
    ...
    targetedFoam(position, num = 1, speed = 21 + 7 * Math.random(), range = 12000, isRandomAim = true) {
        let shotsFired = 0
        const targets = [] //target nearby mobs
        for (let i = 0, len = mob.length; i < len; i++) {
            const dist = Vector.magnitude(Vector.sub(position, mob[i].position));
    ...
    foam(position, velocity, radius) {
        if (tech.isFoamCavitation && Math.random() < 0.99) {
            velocity = Vector.mult(velocity, 1.35)
            radius = 1.2 * radius + 13
        } else if (tech.isFoamCavitation) {
            velocity = Vector.mult(velocity, 1.35)
            radius = 1.2 * radius + 13
        }
        if (Math.random() < 0.99) {
            velocity = Vector.mult(velocity, 2)
            radius = 3 * radius + 39
        } else {
            velocity = Vector.mult(velocity, 2)
            radius = 3 * radius + 39
        ...
        bullet[me] = Bodies.polygon(position.x, position.y, 20, radius, {
            ...
            scale: 1 - 0.001 / tech.bulletsLastLonger * (tech.isFastFoam ? 1.65 : 1),
            classType: "bullet",
            collisionFilter: {
                category: cat.bullet,
    ...
    nail(pos, velocity, dmg = 1) {
        dmg *= tech.bulletSize
        const me = bullet.length;
        bullet[me] = Bodies.rectangle(pos.x, pos.y, 200 * tech.bulletSize, 16 * tech.bulletSize, b.fireAttributes(Math.atan2(velocity.y, velocity.x)));
        Matter.Body.setVelocity(bullet[me], velocity);
        Composite.add(engine.world, bullet[me]); //add bullet to world
    ...
    needle(angle = m.angle) {
        const me = bullet.length;
        bullet[me] = Bodies.rectangle(m.pos.x + 40 * Math.cos(m.angle), m.pos.y + 40 * Math.sin(m.angle), 75 * 8 * tech.bulletSize, 0.75 * 8 * tech.bulletSize, b.fireAttributes(angle));
    ...
    foamBot(position = { x: player.position.x + 50 * (Math.random() - 0.5), y: player.position.y + 50 * (Math.random() - 0.5) }, isKeep = true) {
        ...
        bullet[me] = Bodies.polygon(position.x, position.y, 6, RADIUS, {
            ...
            cd: 0,
            fireCount: 0,
            fireLimit: 20 + 8 * tech.isFoamBotUpgrade,
    ...
    orbitBot(position = player.position, isKeep = true) {
        ...
        const me = bullet.length;
        bullet[me] = Bodies.polygon(position.x, position.y, 9, 36, {
    ...
    guns: [
        {
            ...
            descriptionFunction() {
                return `use compressed fart juice to rapidly drive giant <strong>nails</strong><br><em>fire rate</em> <strong>increases</strong> the longer you fire<br><strong>${this.ammoPack.toFixed(0)}</strong> nails per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 27,
            defaultAmmoPack: 27,
            ...
            chooseFireMethod() { //set in simulation.startGame
                if (tech.nailRecoil) {
                    if (tech.iss) {
                        this.fire = this.fireRecoils
                    } else {
                        this.fire = this.fireRecoilNails
                    }
                } else if (tech.iss) {
                    this.fire = this.fires
            ...
            fireRecoilNails() {
                ...
                if (m.onGround) {
                    ...
                    player.force.y -= 0.09 * Math.sin(m.angle) //reduce knock back in vertical direction to stop super jumps
                }
            },
            fireNormal() {
                if (this.nextFireCycle + 1 < m.cycle) this.startingHoldCycle = m.cycle //reset if not constantly firing
            ...
            fireRivets() {
                ...
                const me = bullet.length;
                const size = tech.bulletSize * 128
                ...
                b.muzzleFlash();
                //very complex recoil system
                // XD lmao at this overcomplicated recoil system intent on preventing super jumps. -AnnonymousNerd87
                // Imma just tweak it and buff the force in the air to encourage super jumps. -AnnonymousNerd87
                if (m.onGround) {
                    if (m.crouch) {
                        const KNOCK = 0.01
                        player.force.x -= KNOCK * Math.cos(m.angle)
                    ...
                    const KNOCK = 0.03
                    player.force.x -= KNOCK * Math.cos(m.angle)
                    player.force.y -= KNOCK * Math.sin(m.angle) * 1.5 //reduce knock back in vertical direction to stop super jumps    
                }
            },
            fireRecoilRivets() {
        ...
        },
        {
            ...
            descriptionFunction() {
                return `fire a wide <strong>burst</strong> of short range <strong> bullets</strong> about as accurate as a blind man pissing during an earthquake <br>has an obsenely fast <strong><em>fire rate</em></strong><br><strong>${this.ammoPack.toFixed(1)}</strong> shots per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 1.6,
            defaultAmmoPack: 1.6,
            ...
            fire() {
                ...
                const coolDown = function () {
                    if (m.crouch) {
                        spread = 1.6
                        m.fireCDcycle = m.cycle + Math.floor((73 / 4 + 36 / 4 * tech.shotgunExtraShots) * b.fireCDscale / 4) // cool down
                        ...
                        m.fireCDcycle = m.cycle + Math.floor((56 / 4 + 28 / 4 * tech.shotgunExtraShots) * b.fireCDscale / 4) // cool down
                ...
                const chooseBulletType = function () {
                    if (tech.isRivets) {
                        ...
                        bullet[me] = Bodies.rectangle(m.pos.x + 35 * Math.cos(m.angle), m.pos.y + 35 * Math.sin(m.angle), 560 * tech.bulletSize, 250 * tech.bulletSize, b.fireAttributes(m.angle));
        ...
        },
        {
            ...
            descriptionFunction() {
                return `emit strangely large <strong>wave packets</strong> that propagate through <strong>solids</strong> quicker than usual <br>waves <strong class='color-s'>slow</strong> mobs<br><strong>${this.ammoPack.toFixed(0)}</strong> wave packets per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 52,
            defaultAmmoPack: 52,
            ...
            fireTransverse() {
                ...
                const me = bullet.length;
                bullet[me] = Bodies.polygon(m.pos.x + 25 * Math.cos(m.angle), m.pos.y + 25 * Math.sin(m.angle), 5, 20, {
                    angle: m.angle,
                    cycle: -0.5,
        ...
        },
        {
            ...
            descriptionFunction() {
                return `launch giant freaking ICBMs that target mobs<br>missiles <strong class='color-e'>explode</strong> on contact with mobs<br><strong>${this.ammoPack.toFixed(1)}</strong> missiles per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 2.3,
            defaultAmmoPack: 2.3,
            ...
            descriptionFunction() {
                return `lob more than a single <strong>bouncy</strong> projectile<br><strong class='color-e'>explodes</strong> on <strong>contact</strong> or after one second<br><strong>${this.ammoPack.toFixed(0)}</strong> grenades per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 3.2,
            defaultAmmoPack: 3.2,
            ...
            fire() {
                ...
                m.fireCDcycle = m.cycle + Math.floor((m.crouch ? 35 : 27) * b.fireCDscale / 3 / countReduction); // cool down
                const where = {
                    x: m.pos.x + 30 * Math.cos(m.angle),
                    y: m.pos.y + 30 * Math.sin(m.angle)
                }
                const SPREAD = m.crouch ? 0.12 : 0.2
                let angle = m.angle - SPREAD * (tech.missileCount + 2) / 2;
                for (let i = -5; i < tech.missileCount; i++) {
        ...
        },
        {
            name: "foam", //8
            descriptionFunction() {
                return `spray terrifying ungodly <strong>foam</strong> that <strong>sticks</strong> to mobs<br><strong class='color-s'>slows</strong> mobs and does <strong class='color-d'>damage</strong> over time<br><strong>${this.ammoPack.toFixed(0)}</strong> bubbles per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ammoPack: 12.6,
        ...
        },
        {
            ...
            descriptionFunction() {
                return `rapidly throw a <strong>self-steering</strong> harpoon that uses <strong class='color-f'>energy</strong><br>to <strong>retract</strong> and refund its <strong class='color-ammo'>ammo</strong> cost<br><strong>${this.ammoPack.toFixed(1)}</strong> harpoons per ${powerUps.orb.ammo()}`
            },
            ammo: 0,
            ...
            railDo() {
                if (this.charge > 0) {
                    ...
                    if (m.energy < DRAIN) {
                        ...
                        // m.energy -= this.drain
                        m.fireCDcycle = m.cycle + 12; // cool down if out of energy
                        this.endCycle = 0;
                        this.charge = 0
                        b.refundAmmo()
                        return
                    ...
                    if ((!input.fire && this.charge > 0.1)) {
                        ...
                        const range = 300 + 1500 * this.charge
                        ...
                        if (tech.extraHarpoons) {
                            ...
                            for (let i = 0, len = mob.length; i < len; ++i) {
                                if (mob[i].alive && !mob[i].isBadTarget && !mob[i].shield && Matter.Query.ray(map, m.pos, mob[i].position).length === 0 && !mob[i].isInvulnerable) {
                                    ...
                                    if (dot > 0.95 - Math.min(dist * 0.00015, 0.3)) { //lower dot product threshold for targeting then if you only have one harpoon //target closest mob that player is looking at and isn't too close to target
                                        // if (this.ammo > -1) {
                                        //     this.ammo--
                                        b.harpoon(where, m.crouch ? null : mob[i], angle, harpoonSize * 3, false, 35, false, thrust) //harpoon(where, target, angle = m.angle, harpoonSize = 1, isReturn = false, totalCycles = 35, isReturnAmmo = true, thrust = 0.1) {
                                        angle += SPREAD
                                        targetCount++
                            ...
                            if (targetCount < tech.extraHarpoons + 1) {
                                ...
                                for (let i = 0; i < num; i++) {
                                    b.harpoon(where, null, angle, harpoonSize * 3, false, 35, false, thrust)
                                    angle += SPREAD
                                }
                            }
                            simulation.updateGunHUD();
                            ...
                            b.harpoon(where, closest.target, m.angle, harpoonSize * 3, false, 35, false, thrust)
                        }
            ...
            harpoonFire() {
                ...
                m.fireCDcycle = m.cycle + 2 + 15 * b.fireCDscale + 60 * (m.energy < 0.05) + tech.extraHarpoons // cool down is set when harpoon bullet returns to player
            ...
            fire() {
                if (m.crouch) {
                    if (tech.isLaserMine) {
                        ...
                        m.fireCDcycle = m.cycle + Math.floor(6.5 * b.fireCDscale); // cool down
                    } else {
                        const pos = {
                            x: m.pos.x + 30 * Math.cos(m.angle),
                            y: m.pos.y + 30 * Math.sin(m.angle)
                        ...
                        m.fireCDcycle = m.cycle + Math.floor(5.5 * b.fireCDscale); // cool down
                    }
                } else {
                    const pos = {
                        x: m.pos.x + 30 * Math.cos(m.angle),
                    ...
                    m.fireCDcycle = m.cycle + Math.floor(3.5 * b.fireCDscale); // cool down
                }
            }
        },
        {
